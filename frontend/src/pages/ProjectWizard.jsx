            ...
            return (
                <WizardShell config={config} step={step} inputs={inputs} onBack={onBack} onNext={handleNext} nextLabel="LOOKS GOOD - CONTINUE NEXT" total={total} showTopNext={false}>
                    <GlassCard style={{ padding: '3.5rem 4rem 2.5rem', minHeight: '60vh', maxWidth: '900px' }}>
                        <h2 style={{ fontSize: '2.5rem', fontWeight: 800, marginBottom: '1.2rem' }}>Review Rental Plan</h2>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem', marginBottom: '2.5rem' }}>
                            <div>
                                <div style={{ fontWeight: 700, color: '#67E8F9', marginBottom: 4 }}>Project Type</div>
                                <div style={{ fontWeight: 900, fontSize: '1.2rem' }}>Rental Homes</div>
                            </div>
                            <div>
                                <div style={{ fontWeight: 700, color: '#67E8F9', marginBottom: 4 }}>Site Type</div>
                                <div style={{ fontWeight: 900, fontSize: '1.2rem' }}>{siteType === 'half' ? 'Half Site' : siteType === 'full' ? 'Full Site' : 'Double Site'}</div>
                            </div>
                            <div>
                                <div style={{ fontWeight: 700, color: '#67E8F9', marginBottom: 4 }}>Plot Dimensions</div>
                                <div style={{ fontWeight: 900, fontSize: '1.2rem' }}>{dimensions}</div>
                            </div>
                            <div>
                                <div style={{ fontWeight: 700, color: '#67E8F9', marginBottom: 4 }}>Floors</div>
                                <div style={{ fontWeight: 900, fontSize: '1.2rem' }}>{floors}</div>
                            </div>
                            <div>
                                <div style={{ fontWeight: 700, color: '#67E8F9', marginBottom: 4 }}>Unit Type</div>
                                <div style={{ fontWeight: 900, fontSize: '1.2rem' }}>{unitType}</div>
                                <div style={{ fontSize: '0.95rem', color: '#a5f3fc', marginTop: 2 }}>
                                    {unitType === 'Single Bedroom' && '1 Bedroom, 1 Bathroom, Kitchen, Small Hall'}
                                    {unitType === 'Double Bedroom' && '2 Bedrooms, 1–2 Bathrooms, Kitchen, Hall'}
                                    {unitType === 'Multiple Bedroom' && '2–3 Bedrooms, 2 Bathrooms, Kitchen, Large Hall'}
                                </div>
                            </div>
                            <div>
                                <div style={{ fontWeight: 700, color: '#67E8F9', marginBottom: 4 }}>Plan</div>
                                <div style={{ fontWeight: 900, fontSize: '1.2rem' }}>{plan}</div>
                            </div>
                            <div>
                                <div style={{ fontWeight: 700, color: '#67E8F9', marginBottom: 4 }}>Semi Interior</div>
                                <div style={{ fontWeight: 900, fontSize: '1.2rem' }}>Included</div>
                            </div>
                            <div>
                        ...
                        <div style={{ fontSize: '1.1rem', color: '#fff', marginBottom: '1.5rem' }}>
                            <b>Note:</b> Rental estimation uses a simplified cost breakdown and prioritizes repeated units, lower interior cost, and ROI-focused development.
                        </div>
                    </GlassCard>
                    <div style={{ width: '92vw', maxWidth: '900px', marginTop: '0.8rem' }}>
                        <BottomStepButton
                            label="CONTINUE NEXT"
                            onClick={handleNext}
                            top="0"
