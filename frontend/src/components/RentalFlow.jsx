...
export default function RentalFlow({
    ...
    const plotSizes = [
        { id: 'half-site', name: 'Half Site', description: '15x30 or 15x40 smaller plot' },
    ...
    useEffect(() => {
        ...
        if (selectedPlotSize) {
            ...
            setShowPlotOptions(true); // Assuming parent wants to show context
            setShowDimensions(true);
        } else {
            setTimeout(() => setShowPlotOptions(true), 500);
        }
    }, []);
    ...
    const handleDimensionClick = (dimensionId) => {
        ...
        if (dimensionId === 'custom') {
            ...
            onAdditionalQuestions({
                projectType: projectType || 'rental-homes',
    ...
    const getTitle = () => {
        switch (projectType) {
            case 'rental-homes': return 'Rental Homes Estimation';
            case 'commercial': return 'Commercial Estimation';
            case 'villa': return 'Villa Estimation';
            case 'interior': return 'Interior Design';
            case 'exterior': return 'Exterior Design';
            case 'own-house': return 'Own House Estimation';
            default: return 'Project Estimation';
        }
    };
    ...
    return (
        <div className="w-full">
            ...
            >
                {/* Selected Plot Size Display - Consistent with Dream House Flow */}
                {!showPlotOptions && step !== 'plot-selection' && (
                    <div className="text-center mb-8 p-6 rounded-2xl bg-white/10 backdrop-blur-md border border-white/20">
                        <h3 className="text-2xl font-bold text-white mb-2">Your Selection</h3>
                        <p className="text-white/90 text-lg">Plot Size: {plotSizes.find(p => p.id === selectedPlotSize)?.name}</p>
                    </div>
                )}

                {/* Plot Selection */}
                {step === 'plot-selection' && showPlotOptions && (
                    <motion.div
                        initial={{ opacity: 0, y: 30 }}
                        animate={{ opacity: 1, y: 0 }}
                    >
                        <h2 className="text-3xl font-bold text-white text-center mb-6">Select the Plot Size</h2>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-6xl mx-auto">
                            {plotSizes.map((plot, index) => (
                                ...
                                >
                                    <h3 className="text-2xl font-bold text-white mb-2">{plot.name}</h3>
                                    <p className="text-white/80">{plot.description}</p>
                                </motion.div>
                            ))}
                        </div>
                    </motion.div>
                ...
                {/* Dimension Selection */}
                {step === 'dimension-selection' && showDimensions && (
                    <motion.div
                        initial={{ opacity: 0, y: 30 }}
                        animate={{ opacity: 1, y: 0 }}
                    >
                        <h2 className="text-3xl font-bold text-white text-center mb-6">Select the Dimension</h2>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-5xl mx-auto">
                            {(() => {
                                ...
                                >
                                    ...
                                    <p className="text-white/70">{dimension.area}</p>
                                </motion.div>
                            ))}
                        </div>
                        ...
                        {showCustomInputs && (
                            ...
                            >
                                <h3 className="text-xl font-bold text-white mb-4">Enter Custom Dimensions</h3>
